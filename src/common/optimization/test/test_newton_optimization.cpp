...
TEST_F(TestNewtonOptimization, newton_optimization_validation) {
  ...
  const NewtonOptimizationOptions * const options = new NewtonOptimizationOptions(OptimizationDirection::TOWARDS_ZERO, max_iter, 0.0, 0.0, 1e-5);
  EXPECT_EQ(options->optimization_direction(), OptimizationDirection::TOWARDS_ZERO);
  ...
  {
    OptimizationSummary summary = no.solve(dummy_optimization_problem, dummy_x0, dummy_x_out, options);
    EXPECT_LE(summary.number_of_iterations_made(), max_iter);
    EXPECT_EQ(summary.convergence_tolerance_criteria_met(), true);
...
TEST_F(TestFixedLineSearch, fixed_line_search_validation) {
  ...
  // test derived class
  FixedLineSearch fls;
  EXPECT_EQ(fls.get_step_max(), std::numeric_limits<float_t>::min());
  fls.set_step_max(step);
  EXPECT_EQ(fls.compute_step_length(dummy_optimization_problem), step);
  EXPECT_EQ(dynamic_cast<LineSearch<FixedLineSearch>*>(&fls)->compute_step_length(dummy_optimization_problem), step);
}
